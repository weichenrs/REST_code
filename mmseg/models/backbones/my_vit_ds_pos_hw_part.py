...
class MYVisionTransformer_ds_pos_hw_part(BaseModule):
    """Vision Transformer.
    ...
    def _pos_embeding(self, patched_img, hw_shape, pos_embed):
        ...
        tmp_size = np.int32(np.sqrt(pos_embed.shape[-2]))
        tmp_dim = pos_embed.shape[-1]
        tmp_embed = pos_embed.view(1, tmp_size, tmp_size, -1)
        sp_times = np.uint8(np.log2(dist.get_world_size()))
        sp_temp = sp_times // 2
        sp_h = 2 ** sp_temp
        sp_w = 2 ** (sp_times - sp_temp)
        ind_h = dist.get_rank()//sp_h
        ind_w = dist.get_rank()%sp_h
        tmp_embed = torch.chunk(tmp_embed, sp_h, dim=-3)[ind_w] if sp_h > 1 else tmp_embed
        tmp_embed = torch.chunk(tmp_embed, sp_w, dim=-2)[ind_h] if sp_w > 1 else tmp_embed
        
        tta_times = hw_shape[0] / (self.img_size[0] // self.patch_size // sp_h)
        if tta_times != 1:
            tmp_embed = tmp_embed.permute(0, 3, 1, 2)
            tmp_embed = resize(tmp_embed, scale_factor=(tta_times, tta_times), 
                               align_corners=False, mode=self.interpolate_mode)
            tmp_embed = tmp_embed.permute(0, 2, 3, 1)
        
        pos_embed = tmp_embed.contiguous().view(1, -1, tmp_dim)
        ...
        # ind_h = dist.get_rank()//sp_h
        # ind_w = dist.get_rank()%sp_h
        # tmp_embed = torch.chunk(tmp_embed, sp_h, dim=-2)[ind_w] if sp_h > 1 else tmp_embed
        # tmp_embed = torch.chunk(tmp_embed, sp_w, dim=-1)[ind_h] if sp_w > 1 else tmp_embed
        # tta_times = hw_shape[0] / (self.img_size[0] // self.patch_size // sp_h)
        # tmp_embed = resize(
        #     tmp_embed, scale_factor=(tta_times, tta_times), align_corners=False, mode=self.interpolate_mode)
        # pos_embed = tmp_embed.contiguous().view(1, -1, tmp_dim)
        
        # pos_embed = torch.chunk(pos_embed, dist.get_world_size(), dim=-2)[dist.get_rank()] 
