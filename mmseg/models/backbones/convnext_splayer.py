...
from mmpretrain.models import GRN, build_norm_layer
...
class ConvNeXt_sp_layer(BaseBackbone):
    ...
    def __init__(self,
        ...
        for i in range(self.num_stages):
            ...
            spattn = MYTransformerEncoderLayer(
                    ...
                    feedforward_channels=4*self.channels[i],
                    attn_drop_rate=0.,
                    drop_rate=0.,
                    drop_path_rate=0.3,
                    num_fcs=2,
    ...
    def forward(self, x):
        ...
        for i, stage in enumerate(self.stages):
            ...
            b, c, h, w = x.shape
            x = rearrange(x, 'b c h w -> b c (h w)', b=b, h=h, w=w, c=c)
            x = rearrange(x, 'b c (h w) -> b (h w) c', b=b, h=h, w=w, c=c)
            if i in self.out_indices:
                x = x + self.spattns[i](x)
    ...
    def train(self, mode=True):
        super(ConvNeXt_sp_layer, self).train(mode)
        self._freeze_stages()
