...
class SwinTransformer_3d(BaseModule):
    ...
    def init_weights(self):
        ...
        for k, v in _state_dict.items():
            k = k.replace('projection', 'proj').replace('backbone.','').replace('stages','layers').replace('.w_msa',
                    '').replace('ffn.layers.0.0','mlp.fc1').replace('ffn.layers.1','mlp.fc2')
            # if k.startswith('backbone.'):
            #     state_dict[k[9:]] = v
            # if k.startswith('patch_embed.projection.'):
            #     state_dict['patch_embed.proj.'] = v
        ...
        state_dict['patch_embed.proj.weight'] = state_dict['patch_embed.proj.weight'].mean(1).unsqueeze(1).unsqueeze(2).repeat(1, 1,
                                                                                                          self.patch_size[
                                                                                                              0], 1,
                                                                                                          1) / \
                                                self.patch_size[0]
        ...
        for k in relative_position_bias_table_keys:
            ...
            if nH1 != nH2:
                print_log(f"Error in loading {k}, passing")
            else:
                if L1 != L2:
                    S1 = int(L1 ** 0.5)
    ...
    def forward(self, x, use_3D_feature=True, use_multiscale_feature=True):
        feature_list = []
        """Forward function."""
        x = x.unsqueeze(1)
        x = self.patch_embed(x) # (B, C, D, H, W) e.g. (B, 1, 32, 256, 256) to (B, 96, 8, 64, 64)

        x = self.pos_drop(x)

        n = 0
        for layer in self.layers:
            feature, x = layer(x.contiguous())
            if use_multiscale_feature:
                feature = self.norm[n](feature)

            feature = rearrange(feature, 'b d h w c -> b c d h w')

            if not use_3D_feature:
