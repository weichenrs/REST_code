...
from .sp_attn_layer import MYViTSelfAttention
...
class TPN_DecoderLayer(TransformerDecoderLayer):
    def __init__(self, **kwargs):
        ...
        # del self.self_attn
        # self.self_attn = MYViTSelfAttention(embed_dims=kwargs['d_model'], num_heads=kwargs['nhead'], 
        #                             dropout=0.1, batch_first=True, bias=True)
...
class Attention(nn.Module):
    ...
    def forward(self, xq, xk, xv):
        ...
        # q = _SeqAllToAll.apply(_SEQUENCE_PARALLEL_GROUP, q, self.scatter_idx, self.gather_idx)
        k = _SeqAllToAll.apply(_SEQUENCE_PARALLEL_GROUP, k, self.scatter_idx, self.gather_idx)
        v = _SeqAllToAll.apply(_SEQUENCE_PARALLEL_GROUP, v, self.scatter_idx, self.gather_idx)
        ...
        x = _SeqAllToAll.apply(_SEQUENCE_PARALLEL_GROUP, x, self.gather_idx, self.scatter_idx)
...
class ATMHead_sp_ds(BaseDecodeHead):
    def __init__(
            self,
            img_size,
            in_channels,
        ...
        super(ATMHead_sp_ds, self).__init__(
        ...
        # self.image_size = (img_size//dist.get_world_size(), img_size)
