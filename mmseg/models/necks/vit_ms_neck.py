...
class VITMSNeck(nn.Module):
    ...
    def forward(self, inputs):
        ...
        #         self.downsample_4x
        #     ]
        if self.rescales == [4, 2, 1, 0.5]:
            ops = [
                self.upsample_4x, self.upsample_2x, self.identity,
                self.downsample_2x]
            
        elif self.rescales == [2, 1, 0.5]:
            ops = [
                self.upsample_2x, self.identity, self.downsample_2x]
            
        elif self.rescales == [4]:
            ops = [
                self.upsample_4x]

        elif self.rescales == [2]:
            ops = [
                self.upsample_2x]

        elif self.rescales == [1, 0.5, 0.25]:
            ops = [
                self.identity, self.downsample_2x, self.downsample_4x]

        for i in range(len(ops)):
            outputs.append(ops[i](inputs[0]) + resize(inputs[0], scale_factor=self.rescales[i], mode='bilinear'))
