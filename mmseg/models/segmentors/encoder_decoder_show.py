...
class EncoderDecoder_show(BaseSegmentor):
    ...
    def input2basket(self, feature_map, gt_cuda):
            ...
            namedir = 'proj/spvit/work_dirs/visattn/0927/512/feat_vecs/'
            os.makedirs(namedir, exist_ok=True)
            np.save(namedir + str(len(os.listdir(namedir))) + '.npy', feat_vecs.numpy())
            
            namedir = 'proj/spvit/work_dirs/visattn/0927/512/feat_vec_labels/'
            os.makedirs(namedir, exist_ok=True)
            np.save(namedir + str(len(os.listdir(namedir))) + '.npy', feat_vec_labels.numpy())

            namedir = 'proj/spvit/work_dirs/visattn/0927/512/feat_vec_domlabels/'
            os.makedirs(namedir, exist_ok=True)
            np.save(namedir + str(len(os.listdir(namedir))) + '.npy', feat_vec_domlabels.numpy())
    ...
    def slide_inference(self, inputs: Tensor,
        ...
        feats = inputs.new_zeros((batch_size, self.decode_head.in_channels[-1], int(h_img/32), int(w_img/32)))
        count_mat = inputs.new_zeros((batch_size, 1, h_img, w_img))
        count_feat = inputs.new_zeros((batch_size, 1, int(h_img/32), int(w_img/32)))
        for h_idx in range(h_grids):
            for w_idx in range(w_grids):
                ...
                # with shape [N, C, H, W]
                crop_seg_logit, crop_feats = self.encode_decode(crop_img, batch_img_metas)
                preds += F.pad(crop_seg_logit,
                               (int(x1), int(preds.shape[3] - x2), int(y1),
                                int(preds.shape[2] - y2)))

                feats += F.pad(crop_feats[-1],
                               ( int(x1/32), int(preds.shape[3]/32 - x2/32), int(y1/32),
                                 int(preds.shape[2]/32 - y2/32) )
                               )
                               
                count_feat[:, :, int(y1/32):int(y2/32), int(x1/32):int(x2/32)] += 1
                count_mat[:, :, y1:y2, x1:x2] += 1
                
        assert (count_mat == 0).sum() == 0
        assert (count_feat == 0).sum() == 0
        seg_logits = preds / count_mat
        feats = feats/ count_feat
        
        namedir = 'proj/spvit/work_dirs/visattn/0927/512featmap/'
        # os.makedirs(namedir, exist_ok=True)
        name = namedir + os.path.basename(batch_img_metas[0]['seg_map_path']).replace('.png', '_feats3.npy')
        np.save(name, feats.detach().cpu().numpy())
