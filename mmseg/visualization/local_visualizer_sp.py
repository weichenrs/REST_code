...
class SegLocalVisualizer_sp(Visualizer):
    """Local Visualizer.
    ...
    def _draw_sem_seg(self,
                      ...
                      withLabels: Optional[bool] = False,
                      split: Optional[bool] = False) -> np.ndarray:
        ...
        if split == True:
            sem_seg = torch.chunk(sem_seg, sp_h, dim=-2)[ind_w] if sp_h > 1 else sem_seg
            sem_seg = torch.chunk(sem_seg, sp_w, dim=-1)[ind_h] if sp_w > 1 else sem_seg
        ...
        if draw_gt and data_sample is not None:
            if 'gt_sem_seg' in data_sample:
                ...
                gt_img_data = self._draw_sem_seg(image, data_sample.gt_sem_seg,
                                                 classes, palette, withLabels, split=True)
