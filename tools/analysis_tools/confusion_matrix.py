...
import sys
sys.path.append('proj/spvit')

from mmseg.datasets.fbp_new import FBPDataset_new
...
def calculate_confusion_matrix(dataset, results):
    ...
    for idx, per_img_res in enumerate(results):
        res_segm = per_img_res.astype(np.int16)
        gt_segm = dataset[idx]['data_samples'] \
            .gt_sem_seg.data.squeeze().numpy().astype(np.int16)
...
def plot_confusion_matrix(confusion_matrix,
    ...
    title_font = {'weight': 'bold', 'size': 40}
